...
class PluginGetUserMedia {
	...
	func call(
		...
		rtcMediaStream = self.rtcPeerConnectionFactory.mediaStream(withLabel: NSUUID().uuidString)
		...
		if videoRequested == true {
			...
			rtcVideoTrack = self.rtcPeerConnectionFactory.videoTrack(withID: NSUUID().uuidString,
		...
		if audioRequested == true {
			...
			rtcAudioTrack = self.rtcPeerConnectionFactory.audioTrack(withID: NSUUID().uuidString)
